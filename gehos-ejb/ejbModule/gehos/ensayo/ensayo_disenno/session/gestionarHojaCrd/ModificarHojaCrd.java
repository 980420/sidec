...
@Name("modificarHojaCrd")
@Scope(ScopeType.CONVERSATION)
public class ModificarHojaCrd {
	@In
	...
	// private String valoresCalculado = "";
	...
	public void inicializarHojaCrd() {
		...
		listaUbicacionRespuesta.add(SeamResourceBundle.getBundle().getString(
				"prm_vertical_ens"));
		this.hojaCrd = (HojaCrd_ensayo) entityManager.find(
				HojaCrd_ensayo.class, idHojaCrd);
		this.secciones = (ArrayList<Seccion_ensayo>) entityManager
				.createQuery(
						"select seccion from Seccion_ensayo seccion "
								+ "where seccion.hojaCrd.id = :idHojaCrd and seccion.eliminado = false")
								.setParameter("idHojaCrd", idHojaCrd).getResultList();
		this.grupos = (ArrayList<GrupoVariables_ensayo>) entityManager
				.createQuery(
						"select grupo from GrupoVariables_ensayo grupo "
								+ "inner join grupo.seccion seccion "
								+ "where seccion.hojaCrd.id = :idHojaCrd and grupo.eliminado = false")
								.setParameter("idHojaCrd", idHojaCrd).getResultList();
		this.variables = (ArrayList<Variable_ensayo>) entityManager
				.createQuery(
						"select variable from Variable_ensayo variable "
								+ "inner join variable.seccion seccion "
								+ "where seccion.hojaCrd.id = :idHojaCrd and variable.eliminado <> true order by variable.id ASC")
								.setParameter("idHojaCrd", idHojaCrd).getResultList();
		this.inicioConversacion = true;

	}

	// Metodo que devuelve el titlo de las secciones
	...
	// Metodo para modificar la hoja CRD.
	public String modificarHojaCrd() {
		...
		hojaCrd.setFechaActualizacion(Calendar.getInstance().getTime());
		...
		for (int i = 0; i < variables.size(); i++) {
			if (existeVariablesNomenclador(variables.get(i))) {
				...
				List list = entityManager.createQuery(" SELECT nv FROM NomencladorValor_ensayo nv WHERE nv.nomenclador.id = :idPA ").setParameter("idPA", variables.get(i).getNomenclador().getId()).getResultList();
				for (Iterator iterator = list.iterator(); iterator
						.hasNext();) {
					NomencladorValor_ensayo object = (NomencladorValor_ensayo) iterator.next();
					Set<NomencladorValor_ensayo> lista = variables.get(i).getNomenclador().getNomencladorValors();
					boolean del = true;
					for (Iterator delete = lista.iterator(); delete
							.hasNext();) {
						NomencladorValor_ensayo nomencladorValor_ensayo = (NomencladorValor_ensayo) delete
								.next();
						if(object.getValor().equalsIgnoreCase(nomencladorValor_ensayo.getValor()))
						{
							del=false;		
							object.setValorCalculado(nomencladorValor_ensayo.getValorCalculado());
							object.setCid(nomencladorValor_ensayo.getCid());	
				...
				list = entityManager.createQuery(" SELECT nv FROM NomencladorValor_ensayo nv WHERE nv.nomenclador.id = :idPA ").setParameter("idPA", variables.get(i).getNomenclador().getId()).getResultList();
				Set<NomencladorValor_ensayo> lista = variables.get(i).getNomenclador().getNomencladorValors();

				for (NomencladorValor_ensayo nuevo : lista) {

					boolean insert = true;
					for (Iterator datnv = list.iterator(); datnv.hasNext();) {
						NomencladorValor_ensayo viejo = (NomencladorValor_ensayo) datnv.next();

						if (nuevo.getValor().equalsIgnoreCase(viejo.getValor())) {
							insert = false;
						}
			...
			variables.get(i).setEliminado(false);
			entityManager.persist(variables.get(i));
		}
	...
	// Metodo que devuelve la seccin dado la etiqueta
	...
	// Metodo que devuelve si una seccin esta asociada a una variable
	...
	// Metodo que devuelve si una seccin esta asociada a un grupo
	...
	// Metodo para eliminar la seccin del listado de secciones
	public void eliminarSeccion(Seccion_ensayo seccion) {
		try {
			if (seccionAsociadaVariable(seccion.getEtiquetaSeccion())) {
				...
				if (existeSeccionEnBaseDatos(seccion)) {
	...
	// Metodo para saber si la seccion estaba almacenada en base de datos.
	public boolean existeSeccionEnBaseDatos(Seccion_ensayo seccion) {
		try {
			if (!seccion.getEliminado().equals(null))
				return true;
			return false;
		} catch (Exception e) {
			return false;
	...
	// Metodo para adicionar el grupo al listado de grupos
		...
		try {
			...
			if (modificandoGrupo) {
				...
				if (grupoListado(etiquetaGrupo) != null) {
					...
					// vert pk no me esta cargando la etiqueta de la seccion en
					// la vista
	...
	public void eliminarGrupo(GrupoVariables_ensayo grupo) {
		try {
			if (grupoAsociado(grupo.getEtiquetaGrupo())) {
				...
				if (existeGrupoEnBaseDatos(grupo)) {
	...
	// Metodo para saber si el grupo estaba almacenada en base de datos.
	public boolean existeGrupoEnBaseDatos(GrupoVariables_ensayo grupo) {
		try {
			if (!grupo.getEliminado().equals(null))
				return true;
			return false;
		} catch (Exception e) {
			return false;
	...
	public void refrescarPresentacion(){
		
		this.setNombrePresentacionFormulario("");
	...
	// Metodos para adicionar y modificar las variables
	...
	public void modificarVariable(Variable_ensayo variable) {
		...
		this.variable.setUnica(variable.getUnica());
		...
		this.variable.setCritica(variable.getCritica());
		this.variable.setNumeroColumna(variable.getNumeroColumna());
		this.variable.setNumeroPregunta(variable.getNumeroPregunta());
		this.variable.setRequerido(variable.getRequerido());
		...
		try {
			...
			if (modificandoVar) {
				...
				if (!error) {
					...
					this.variables.get(posVarModificar).setUnica(
							variable.getUnica());
					this.variables.get(posVarModificar).setCritica(
							variable.getCritica());
					...
					if (habilitarNomenclador) {
						...
						nomencladorVariable.getNomencladorValors().clear();							
				...
				if (variableListado(nombreVariable) != null) {
					...
					if (habilitarNomenclador) {
						...
						for (int i = 0; i < valoresEnTexto.length; i++) {
							...
							// en
							// texto
							nomencladoValor.setValorCalculado(valorCalculado);// valor calculado
							nomencladoValor.setNomenclador(nomencladorVariable);
							nomencladorVariable.getNomencladorValors().add(
									nomencladoValor);
						}
	...
	public void eliminarVariable(Variable_ensayo variable) {
		try {
			if (existeVariableEnBaseDatos(variable)) {
				variable.setEliminado(true);
	...
	// Metodo para saber si el grupo estaba almacenada en base de datos.
	public boolean existeVariableEnBaseDatos(Variable_ensayo variable) {
		try {
			if (!variable.getEliminado().equals(null))
				return true;
			return false;
		} catch (Exception e) {
			return false;
	...
	// abilitar modificar y eliminar si momento de seguimiento asociado

	@SuppressWarnings("unchecked")
	public boolean habilitarModificarEliminar(HojaCrd_ensayo hojap) {
		boolean result = false;
		List<MomentoSeguimientoGeneral_ensayo> listams = new ArrayList<MomentoSeguimientoGeneral_ensayo>();
		MomentoSeguimientoGeneral_ensayo mss = new MomentoSeguimientoGeneral_ensayo();

		listams = (List<MomentoSeguimientoGeneral_ensayo>) entityManager
				.createQuery(
						"select ms from MomentoSeguimientoGeneral_ensayo ms "
								+ "inner join ms.momentoSeguimientoGeneralHojaCrds msh "
								+ "inner join msh.hojaCrd h "
								+ "where h.id=:hoja")
								.setParameter("hoja", hojap.getId()).getResultList();
		if (listams.size() > 0) {
			result = true;
		}
