...
@Name("tipoFuncionarioValidarControlador")
public class TipoFuncionarioValidarControlador {
	
	@In EntityManager entityManager;
	@In FacesMessages facesMessages;
	@In(required = false, value = "tipoFuncionarioModificarControlador", scope = ScopeType.CONVERSATION)
	TipoFuncionarioModificarControlador tipoFuncionarioModificarControlador;	
	...
	public void valor(FacesContext context, UIComponent component, Object value) {
		if (context.getExternalContext().getRequestParameterMap().containsKey(
			...
			if(tipoFuncionarioModificarControlador != null){
				/*Validating that the rol has not been previously created*/
				lr = entityManager.createQuery("select t from TipoFuncionario_configuracion t where t.valor =:valor and t.id <>:id")
								  .setParameter("valor", value.toString().trim())		
								  .setParameter("id", tipoFuncionarioModificarControlador.getTipoFuncionario().getId())	
								  .getResultList();				
			}
			else{
				/*Validating that the description of location type has not been previously created*/
				lr = entityManager.createQuery("select t from TipoFuncionario_configuracion t where t.valor =:valor")
								  .setParameter("valor", value.toString().trim())										  
								  .getResultList();				
			}
			if(lr.size() != 0){				
