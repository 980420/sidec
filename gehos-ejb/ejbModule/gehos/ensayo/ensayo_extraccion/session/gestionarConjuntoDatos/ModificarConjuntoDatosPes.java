...
@Name("modificarConjuntoDatosPes")
@Scope(ScopeType.CONVERSATION)
public class ModificarConjuntoDatosPes
{
	@In
	...
	private List<String> listadoIdMomentos = new ArrayList();
	private List<String> listadoIdVariables = new ArrayList();
	...
	public ModificarConjuntoDatosPes() {}
	...
	public void inicializarConjuntoDatos() 
	{
		...
		listadoVariables = new ListadoControler_ensayo(listadoVariablesEstudio);
	...
	public void cargarConjuntoDatos() {
		...
		List<Object> variablesAux = entityManager.createQuery("select variable, momentoSeguimientoGeneral from Variable_ensayo variable inner join variable.seccion seccion inner join seccion.hojaCrd hojaCrd inner join hojaCrd.momentoSeguimientoGeneralHojaCrds momentoSeguimientoGeneralHojaCrds inner join momentoSeguimientoGeneralHojaCrds.momentoSeguimientoGeneral momentoSeguimientoGeneral where momentoSeguimientoGeneralHojaCrds.eliminado <> true and str(variable.id) in (#{modificarConjuntoDatosPes.listadoIdVariables}) and str(momentoSeguimientoGeneral.id) in (#{modificarConjuntoDatosPes.listadoIdMomentos}) ORDER BY variable.id")
					.getResultList();
	...
	public String modificarConjuntoDatos() {
		try {
			...
			if(conjuntoDatos.getFechaFin()==null)
				conjuntoDatos.setFechaFin(Calendar.getInstance().getTime());
	...
	public String declaracionHql()
	{
		...
		String hql = "select variableDato from VariableDato_ensayo variableDato "
				...
				+ "inner join crdEspecifico.momentoSeguimientoEspecifico momentoSeguimientoEspecifico "
		...
		for (int i = 0; i < listadoVariablesSeleccionadas.size(); i++) {
			Long idVariavle = Long.valueOf(((VariableConjuntoDatos)listadoVariablesSeleccionadas.get(i)).getVariable().getId());
			Long idMomento = Long.valueOf(((VariableConjuntoDatos)listadoVariablesSeleccionadas.get(i)).getMomentoSeguimientoGeneral().getId());
			if (i == 0) {
		...
		hql = 
				hql + "and momentoSeguimientoGeneral.id in (" + momentos + ") and variable.id in (" + variables + ") " 
