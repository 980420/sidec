...
@Name("modificarPreference") 
public class ModificarPreference {
	...
	private Boolean pass_desigual = true;//validar que las constrasenas sean iguales
	...
	public String cambiar() throws NoSuchAlgorithmException{
		if (!render || !nombrePhoto.equals(nombreFoto ) || !theme.equals(themeVerif) || !culturaSelec.equals(culturaSelecVerif) || !treeLikeMenu.equals(treeLikeMenuVerif)) {
			...
			if (!this.nombrePhoto.equals("")) {	
				...
				try {
					...
					facesMessages.addToControlFromResourceBundle("Acept",
							Severity.ERROR, "Se ha cambiado su foto satisfactoriamente.");*/
			...
			for (int i = 0; i < themeSelector.getThemes().size(); i++) {
				if (themeSelector.getThemes().get(i).getValue().toString().equals(theme)) {
			...
			for (int i = 0; i < listaCultura.size(); i++) {
				if(listaCultura.get(i).getIdioma().equals(culturaSelec)){
					usuario.getProfile().setLocaleString(listaCultura.get(i).getLocalString());
			...
			user.getProfile().setTreeLikeMenu(usuario.getProfile().getTreeLikeMenu());
			
			
			Profile_configuracion profile = entityManager.find(Profile_configuracion.class, this.user.getProfile().getId());
			...
			/*facesMessages.addToControlFromResourceBundle("aceptar",
					Severity.ERROR, "Se ha actualizado su perfil satisfactoriamente.");*/
			...
			facesMessages.addToControlFromResourceBundle("aceptar",
					Severity.ERROR, "mensaje de lorena");
			return "fail";
		}
