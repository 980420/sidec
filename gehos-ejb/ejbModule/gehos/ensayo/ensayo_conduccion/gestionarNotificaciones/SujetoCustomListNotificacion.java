...
@Name("sujetoCustomListNotificacion")
@Scope(ScopeType.CONVERSATION)
public class SujetoCustomListNotificacion extends SujetoList_ensayo {

	private static final String EJBQL = "select sujeto from Sujeto_ensayo sujeto "
			+ "where sujeto.eliminado = false and sujeto.id in (select distinct idSujeto from Notificacion_ensayo n) "
			+ "and sujeto.grupoSujetos.estudio = #{sujetoCustomListNotificacion.seguridadEstudio.getEstudioEntidadActivo().getEstudio()} ";

	private static final String[] RESTRICTIONS = {
			"lower(sujeto.codigoPaciente) like concat('%',concat(lower(#{sujetoCustomListNotificacion.sujeto.codigoPaciente.trim()}),'%'))",
			"lower(sujeto.grupoSujetos.nombreGrupo) like concat(lower(#{sujetoCustomListNotificacion.nombeGrup.trim()}),'%')",
			"lower(sujeto.estadoInclusion.nombre) like concat(lower(#{sujetoCustomListNotificacion.estadoI.trim()}),'%')",
			"lower(sujeto.estadoTratamiento.nombre) like concat(lower(#{sujetoCustomListNotificacion.estadoT.trim()}),'%')",
			"lower(sujeto.fechaCreacion) like concat(lower(#{sujetoCustomListNotificacion.fechaInicioA.trim()}),'%')",
			"lower(sujeto.fechaCreacion) like concat(lower(#{sujetoCustomListNotificacion.fechaInicioP.trim()}),'%')",
			"#{sujetoCustomListNotificacion.idSujeto} <> sujeto.id" };
	...
	public List<Sujeto_ensayo> getResultList() {
		...
		if (!this
			...
			String tempEJB = "select sujeto from Sujeto_ensayo sujeto "
					+ "where sujeto.eliminado = false and sujeto.estadoInclusion.codigo = 4 "
					+ "and sujeto.id in (select distinct n.idSujeto from Notificacion_ensayo n) "
					+ "and sujeto.grupoSujetos.estudio = #{sujetoCustomListNotificacion.seguridadEstudio.getEstudioEntidadActivo().getEstudio()} "
					+ "and sujeto.entidad = #{sujetoCustomListNotificacion.getHospitalActivo()} ";
	...
	public SujetoCustomListNotificacion() {
		setEjbql(EJBQL);
