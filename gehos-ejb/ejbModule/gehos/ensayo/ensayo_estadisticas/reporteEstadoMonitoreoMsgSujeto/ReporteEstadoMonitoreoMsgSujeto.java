...
import gehos.ensayo.ensayo_estadisticas.cantMSxSujeto.cantMSxSujetoSource;
...
@Name("reporteEstadoMonitoreoMsgSujeto")
@Scope(ScopeType.CONVERSATION)
public class ReporteEstadoMonitoreoMsgSujeto{
	...
	List<MonitoreoMsgSujeto> listaEstMonMSxS = new ArrayList<MonitoreoMsgSujeto>(); 
	...
	private Map reporteEstadoMonitoreoMsgSujeto;
	...
	//RF4 - Generar reporte de cumplimiento del cronograma por sujetos
	public void reporteEstadoMonitoreoMsgSujeto()	{
		
		Date fechaActual=Calendar.getInstance().getTime();
		listaEstMonMSxS.clear();
		style = -1;
		...
		if(fechaFin != null && fechaInicio != null && !this.estudio.equals("")) {
				...
				for (int i = 0; i < grupos.size(); i++) {
					...
					if(!sujetosDelGrupo.isEmpty()) {
						...
						for(int k=0;k<sujetosDelGrupo.size();k++) {
							List<MomentoSeguimientoEspecifico_ensayo> momentosSujeto = new ArrayList<MomentoSeguimientoEspecifico_ensayo>();															
							sujeto=sujetosDelGrupo.get(k);
							
							if (entityManager.find(Sujeto_ensayo.class,sujeto.getId()).getFechaInterrupcion() != null){
								List<MomentoSeguimientoEspecifico_ensayo> momentosProgramados = entityManager
										.createQuery(
												"select momento from MomentoSeguimientoEspecifico_ensayo momento where momento.sujeto.id =:idSujeto and momento.momentoSeguimientoGeneral.programado = 'true' and momento.eliminado = false and momento.fechaInicio <= momento.sujeto.fechaInterrupcion")
										.setParameter("idSujeto", sujeto.getId()).getResultList();
								List<MomentoSeguimientoEspecifico_ensayo> momentosNoProgramados = entityManager
										.createQuery(
										...
										.setParameter("idSujeto", sujeto.getId()).getResultList();
								momentosSujeto.addAll(momentosProgramados);
								momentosSujeto.addAll(momentosNoProgramados);
							}
							else{
								momentosSujeto = entityManager
										.createQuery(
												"select momento from MomentoSeguimientoEspecifico_ensayo momento where momento.sujeto.id =:idSujeto and momento.eliminado = false and momento.fechaInicio <=:fechaActual")
							...
							for(int l=0;l<momentosSujeto.size();l++) {
								...
								if(momentosSujeto.get(l).getEstadoMomentoSeguimiento().getCodigo()==1)
									estadoMs = "Iniciado";
								else if(momentosSujeto.get(l).getEstadoMomentoSeguimiento().getCodigo()==2)
									estadoMs = "No iniciado";
								else if(momentosSujeto.get(l).getEstadoMomentoSeguimiento().getCodigo()==3)
									estadoMs = "Completado";
								else if(momentosSujeto.get(l).getEstadoMomentoSeguimiento().getCodigo()==4)
									estadoMs = "Atrasado";
								else if(momentosSujeto.get(l).getEstadoMomentoSeguimiento().getCodigo()==5)
									estadoMs = "Firmado"; 
								
									MonitoreoMsgSujeto estadoCronogramaSujeto = new MonitoreoMsgSujeto(listaEntidadEst.get(z).getEntidad().getNombre(), sujeto.getCodigoPaciente(), momentosSujeto.get(l).getMomentoSeguimientoGeneral().getNombre(), estadoMs, momentosSujeto.get(l).getEstadoMomentoSeguimiento().getCodigo().toString());
									listaEstMonMSxS.add(estadoCronogramaSujeto);
							}					
						}						
					}				
				}									
			...
			if (haySujetos){						
				...
				reporteEstadoMonitoreoMsgSujeto=new HashMap();
				reporteEstadoMonitoreoMsgSujeto.put("nombreEstudio", this.estudio);
				reporteEstadoMonitoreoMsgSujeto.put("totalSujetos", cantSujetos);
				nombreReport=reportManager.ExportReport("estadoMonitoreoMsgSujeto", reporteEstadoMonitoreoMsgSujeto, listaEstMonMSxS, FileType.HTML_FILE);
				flag=true;
				flag2=false;
			} 
			else {
	...
	public void exportReportToFileFormat() {
		...
		if (fileformatToExport.equals(filesFormatCombo.get(0))) {
			pathExportedReport = reportManager.ExportReport(
					"estadoMonitoreoMsgSujeto", reporteEstadoMonitoreoMsgSujeto, listaEstMonMSxS,
					FileType.PDF_FILE);
