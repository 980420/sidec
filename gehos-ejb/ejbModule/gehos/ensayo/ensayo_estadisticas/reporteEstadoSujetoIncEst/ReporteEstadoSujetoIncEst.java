...
@Name("reporteEstadoSujetoIncEst")
@Scope(ScopeType.CONVERSATION)
public class ReporteEstadoSujetoIncEst{
	...
	List<EstadoSujetoIncEst> estadoSujetos;
	...
	private Map reporteEstadoSujetosIncEst;
	...
	//RF5 - Generar reporte del estado de los sujetos incluidos en el estudio
	public void reporteEstadoSujetoIncEst()	{
		
		estadoSujetos = new ArrayList<EstadoSujetoIncEst>();
		style = -1;
		...
		if(fechaFin != null && fechaInicio != null && !this.estudio.equals("")){
			...
			for (int z = 0; z < listaEntidadEst.size(); z++) {
				...
				for (int i = 0; i < grupos.size(); i++) {
					...
					//*****FALTA AGREGAR CONDICION DE SOLO SUJETOS INCLUIDOSSS******
					...
					if(!sujetosDelGrupo.isEmpty()) {
						...
						for(int k=0;k<sujetosDelGrupo.size();k++) {
							...
							String fechaConsentimiento = "";
							String fechaInclusion = "";
							String inicioTratamiento = "";
							String fechaInterrupcion = "";
							String causaInterrupcion = "";
							String fechaFallecimiento = "";
							String causaFallecimiento = "";
							
							
							fechaInclusion = sujeto.getFechaInclucion().toString();
							if(sujeto.getFechaInicronograma() != null)
								inicioTratamiento = formatter.format(sujeto.getFechaInicronograma());
							
							//Buscar los MS de Pesquisaje/Interrupcion/Fallecimiento y obtener las fecha y las causa														
								...
								if(momento.getMomentoSeguimientoGeneral().getNombre().contains("Pesquisaje")){
									...
									for(int m=0; m < crdMomento.size(); m++) {
										...
										if(!hojaCrdDesasociadaEnMS(crdMomento.get(m).getHojaCrd().getId(),momento.getMomentoSeguimientoGeneral().getId())) {
											...
											for(int n=0; n < variables.size(); n++){
												if(variables.get(n).getVariable().getNombreVariable().equals("Fecha del Consentimiento Informado"))
													fechaConsentimiento=variables.get(n).getValor().toString();																							
											}												
										}
									}
								}
								else if(momento.getMomentoSeguimientoGeneral().getNombre().contains("Interrupción")){
									List<CrdEspecifico_ensayo> crdMomento = entityManager
											.createQuery(
													"select crd from CrdEspecifico_ensayo crd where crd.momentoSeguimientoEspecifico.id =:idMomento and crd.eliminado = false")
											.setParameter("idMomento", momento.getId()).getResultList();
									for(int m=0; m < crdMomento.size(); m++) {
										...
										if(!hojaCrdDesasociadaEnMS(crdMomento.get(m).getHojaCrd().getId(),momento.getMomentoSeguimientoGeneral().getId())) {
											...
											for(int n=0; n < variables.size(); n++){
												if(variables.get(n).getVariable().getNombreVariable().equals("Finterruptto"))
													fechaInterrupcion=variables.get(n).getValor().toString();
												else if(variables.get(n).getVariable().getNombreVariable().equals("Causa de interrupción del tratamiento") ||
														variables.get(n).getVariable().getNombreVariable().equals("causainterrupcion") ||
														variables.get(n).getVariable().getNombreVariable().equals("Causainterrupcion") ||
														variables.get(n).getVariable().getNombreVariable().equals("causainterr") ||
														variables.get(n).getVariable().getNombreVariable().equals("causainterrumpe") ||
														variables.get(n).getVariable().getNombreVariable().equals("causasinterrupcion") ||
														variables.get(n).getVariable().getNombreVariable().equals("Causattotratamiento"))													
													causaInterrupcion = variables.get(n).getValor().toString();												
											}												
										}
									}
								}
								else if(momento.getMomentoSeguimientoGeneral().getNombre().contains("Fallecimiento")){
									List<CrdEspecifico_ensayo> crdMomento = entityManager
											.createQuery(
													"select crd from CrdEspecifico_ensayo crd where crd.momentoSeguimientoEspecifico.id =:idMomento and crd.eliminado = false")
											.setParameter("idMomento", momento.getId()).getResultList();
									for(int m=0; m < crdMomento.size(); m++) {
										...
										if(!hojaCrdDesasociadaEnMS(crdMomento.get(m).getHojaCrd().getId(),momento.getMomentoSeguimientoGeneral().getId())) {
											...
											for(int n=0; n < variables.size(); n++){
												if(variables.get(n).getVariable().getNombreVariable().equals("Fechafallecimiento"))
													fechaFallecimiento=variables.get(n).getValor().toString();
												else if(variables.get(n).getVariable().getNombreVariable().equals("causa"))													
													causaFallecimiento = variables.get(n).getValor().toString();																								
											}												
										}
									}
								} 
							...
							EstadoSujetoIncEst estadoSujeto = new EstadoSujetoIncEst(sujeto.getCodigoPaciente(),fechaConsentimiento,fechaInclusion,inicioTratamiento,fechaInterrupcion,causaInterrupcion,fechaFallecimiento,causaFallecimiento);
			...
			if (haySujetos) {
				this.style = 1;
				reporteEstadoSujetosIncEst=new HashMap();
				reporteEstadoSujetosIncEst.put("nombreEstudio", this.estudio);
				reporteEstadoSujetosIncEst.put("totalSujetos", String.valueOf(cantSujetos));
				reporteEstadoSujetosIncEst.put("nombreSujeto", SeamResourceBundle.getBundle().getString("nombreSujeto"));
				reporteEstadoSujetosIncEst.put("fechaConsentimiento", SeamResourceBundle.getBundle().getString("fechaConsentimiento"));
				reporteEstadoSujetosIncEst.put("fechaInclusion", SeamResourceBundle.getBundle().getString("fechaInclusion"));
				reporteEstadoSujetosIncEst.put("inicioTratamiento", SeamResourceBundle.getBundle().getString("inicioTratamiento"));
				reporteEstadoSujetosIncEst.put("fechaInterrupcion", SeamResourceBundle.getBundle().getString("fechaInterrupcion"));	
				reporteEstadoSujetosIncEst.put("causaInterrupcion", SeamResourceBundle.getBundle().getString("causaInterrupcion"));
				reporteEstadoSujetosIncEst.put("fechaFallecimiento", SeamResourceBundle.getBundle().getString("fechaFallecimiento"));
				reporteEstadoSujetosIncEst.put("causaFallecimiento", SeamResourceBundle.getBundle().getString("causaFallecimiento"));
				nombreReport=reportManager.ExportReport("reportEstadoSujetoIncEst", reporteEstadoSujetosIncEst, estadoSujetos, FileType.HTML_FILE);
	...
	public void exportReportToFileFormat() {
		...
		if (fileformatToExport.equals(filesFormatCombo.get(0))) {
			pathExportedReport = reportManager.ExportReport(
					"reportEstadoSujetoIncEst", reporteEstadoSujetosIncEst, estadoSujetos,
					FileType.PDF_FILE);
