...
@Name("modificarSujetoPesquisaje")
@Scope(ScopeType.CONVERSATION)
public class ModificarSujetoPesquisaje {
	...
	public VariableDato_ensayo ObtenerDatoVariable() {
		...
		try {
			dato = (VariableDato_ensayo) entityManager.createQuery(queryCRD)
					.setParameter("idHoja", ObtenerCRD().getHojaCrd().getId())
					.setParameter("idCRD", ObtenerCRD().getId())
					.getSingleResult();
		} catch (Exception e) {
			return null;
		}
	...
	public VariableDato_ensayo ObtenerDatoVariableFecha() {
		...
		try {
			dato = (VariableDato_ensayo) entityManager
					.createQuery(queryCRD)
					.setParameter("idHoja", ObtenerCRD().getHojaCrd().getId())
					.setParameter(
							"fechaInc",
	...
	public List<String> listarEstados1() {
		...
		for (int i = 0; i < listarEstados.size(); i++) {
			if (DevolverRol().getCodigo().equals("ecCord")) {
				if (listarEstados.get(i).getNombre().equals("No Incluido")
						|| listarEstados.get(i).getNombre().equals("Incluido")) {
					nombreEst.add(listarEstados.get(i).getNombre());
				}
			} else if (DevolverRol().getCodigo().equals("ecMon")) {
				if (listarEstados.get(i).getNombre().equals("Mal Incluido")
						|| listarEstados.get(i).getNombre().equals("Incluido")) {
					nombreEst.add(listarEstados.get(i).getNombre());
				}
			}
	...
	public void persistir() {
		if (!this.errores()) {
			...
			/*
			 * EstadoSujeto_ensayo estadosSuje = (EstadoSujeto_ensayo)
			...
			if (numeroInclucion == null) {
				this.sujeto.setCodigoPaciente(inicialesCentroPromotor
						+ "_"
						+ this.seguridadEstudio.getEstudioEntidadActivo()
						.getEstudio().getIdentificador() + "_"
						+ inicialesCentro + "_" + inicialesP);
			} else {
				this.sujeto.setCodigoPaciente(inicialesCentroPromotor
						+ "_"
						+ this.seguridadEstudio.getEstudioEntidadActivo()
								.getEstudio().getIdentificador() + "_"
						+ inicialesCentro + "_" + inicialesP + "_"
						+ numeroInclucion);
			}
			...
			for (int i = 0; i < listadoMomentosGeneral.size(); i++) {
				...
				if ((DevolverRol().getCodigo().equals("ecCord") || DevolverRol()
						.getCodigo().equals("ecInv"))
						&& this.sujeto.getEstadoInclusion().getId() == 4
						&& momentoEvaluacion != null) {
				...
				if ((DevolverRol().getCodigo().equals("ecCord") || DevolverRol()
						...
						&& this.sujeto.getEstadoInclusion().getId() == 4
						&& momentoEvaluacion != null
						&& this.sujeto.getGrupoSujetos().getId() != grupoViejo
								.getId()) {
					...
					entityManager.flush();
					@SuppressWarnings("unchecked")
					List<CrdEspecifico_ensayo> lista = (List<CrdEspecifico_ensayo>) entityManager
							.createQuery(
									"select crd from CrdEspecifico_ensayo crd where crd.momentoSeguimientoEspecifico.id=:id and crd.eliminado = false")
							.setParameter("id", momentoEvaluacion.getId())
							.getResultList();
					...
					// estadoMonitoreo=(EstadoMonitoreo_ensayo)
					...
					EstadoMomentoSeguimiento_ensayo estadoMomento = entityManager
							.find(EstadoMomentoSeguimiento_ensayo.class,
									idEstadoSeguimiento);
					...
					List<HojaCrd_ensayo> listaHojas = entityManager
							...
							.setParameter("id", listadoMomentosGeneral.get(i).getId())
							.getResultList();
					// List<HojaCrd_ensayo>
					...
					momentoEspecifico.setFechaInicio(fechaInclucion);
					cal.add(Calendar.DATE,
							listadoMomentosGeneral.get(i).getTiempoLlenado());
					momentoEspecifico.setFechaFin(cal.getTime());
					momentoEspecifico
							.setMomentoSeguimientoGeneral(listadoMomentosGeneral.get(i));
			...
			this.facesMessages.addToControlFromResourceBundle("idForm",
					Severity.INFO, "sujetoRegistrado");
			// String mensage =
			// this.facesMessages.add(SeamResourceBundle.getBundle().getString("sujetoRegistrado"));
		}
	}
	...
	public boolean errores() {
		...
		String codigoSujeto = "";
		if (numeroInclucion == null) {
			codigoSujeto = inicialesCentroPromotor
					+ "_"
					+ this.seguridadEstudio.getEstudioEntidadActivo()
							.getEstudio().getIdentificador() + "_"
					+ inicialesCentro + "_" + inicialesP;
		...
		// validando que el sujeto no este registrado
		try {
			...
			if (!codigoSujeto.equals(codigoViejo)) {
				listaSujeto = (List<Sujeto_ensayo>) entityManager
						.createQuery(
								"select s from Sujeto_ensayo s where s.codigoPaciente= :codigo and sujeto.eliminado = false")
						.setParameter("codigo", codigoSujeto).getResultList();
			}
			if (listaSujeto.size() > 0) {
				error = true;
				return true;
			}
		} catch (Exception e) {
	...
	public void setEstado(String estado) {
		...
		this.estado = estado;
		if (this.estado.equals("No Incluido")
				|| this.estado.equals(SeamResourceBundle.getBundle().getString(
						"evaluacion"))) {
