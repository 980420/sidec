...
@Name("bioanalistaList_configuracionCustom")
@Scope(ScopeType.CONVERSATION)
public class BioanalistaList_Custom extends
		EntityQuery<Bioanalista_configuracion> {

	private static final String EJBQL = "select medico from Bioanalista_configuracion medico where medico.eliminado = false";

	private static final String[] RESTRICTIONSA = {
			"lower(medico.matriculaColegio) like concat(lower(#{bioanalistaList_configuracionCustom.matriculaColegio.trim()}),'%')",
			"lower(medico.usuario.nombre) like concat(lower(#{bioanalistaList_configuracionCustom.nombre.trim()}),'%')",
			"lower(medico.usuario.primerApellido) like concat(lower(#{bioanalistaList_configuracionCustom.apellido.trim()}),'%')",
			"lower(medico.usuario.segundoApellido) like concat(lower(#{bioanalistaList_configuracionCustom.segundoapellido.trim()}),'%')",
			"#{bioanalistaList_configuracionCustom.idmedico} <> medico.id" };

	private static final String[] RESTRICTIONS = { "#{bioanalistaList_configuracionCustom.idmedico} <> medico.id" };
	...
	public String seleccionarModificar() throws IllegalStateException,
			SecurityException, SystemException {
		try {
			error = 0;
			List<Bioanalista_configuracion> medConcu = entityManager
					.createQuery(
							"select med from Bioanalista_configuracion med where med.id =:id and med.eliminado = true")
					.setParameter("id", idmedico).getResultList();
			if (medConcu.size() != 0) {
	...
	@SuppressWarnings("unchecked")
	public String seleccionarVer() throws IllegalStateException,
			SecurityException, SystemException {
		try {
			error = 0;
			List<Bioanalista_configuracion> medConcu = entityManager
	...
	@SuppressWarnings("unchecked")
	public String seleccionarVer(Long idmedico) throws IllegalStateException,
			SecurityException, SystemException {
		try {
			List<Bioanalista_configuracion> medConcu = entityManager
					.createNamedQuery(
							"select med from Bioanalista_configuracion med where med.id =:id and med.eliminado = false")
					.setParameter("id", idmedico).getResultList();
			if (medConcu.size() != 0) {
				facesMessages.addToControlFromResourceBundle("userSearch",
						Severity.ERROR, "eliminado");
	...
	public void eliminar() {
		try {
			Bioanalista_configuracion aux = (Bioanalista_configuracion) entityManager
					.createQuery(
							"select med from Bioanalista_configuracion med where med.id =:id and med.eliminado =false")
					.setParameter("id", ideliminarMed).getSingleResult();
			this.idmedico = -1l;
			aux.setEliminado(true);
			entityManager.persist(aux);

			List<BioanalistaInEntidad_configuracion> medicoInEntidad = entityManager
					.createQuery(
							"select medInEnt from BioanalistaInEntidad_configuracion medInEnt where medInEnt.bioanalista.id =:id")
					.setParameter("id", ideliminarMed).getResultList();
	...
	@SuppressWarnings("unchecked")
	public void seleccionar() throws IllegalStateException, SecurityException,
			SystemException {
		try {
			List<Bioanalista_configuracion> medConcu = entityManager
					.createQuery(
