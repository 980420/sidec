...
public class CrearSujeto {
	...
	public void listarGrupo() {
		// Obtengo la lista de grupos de sujetos que estan habilitados y no estan borrados
		// en ese estudio activo
	...
	public List<String> listarEstados1() {
		...
		for (int i = 0; i < listarEstados.size(); i++) {
			if (listarEstados.get(i).getNombre().equals("Incluido")) {
	...
	public void persistir() {
		if (!this.errores()) {
			...
			/*@SuppressWarnings("unchecked")
			List<Long> numeros = (List<Long>) entityManager
					.createQuery(
							"select varDato.numeroInclucion from Sujeto_ensayo varDato where varDato.entidad=:Entidad and varDato.numeroInclucion != null and varDato.eliminado = 'FALSE' and varDato.grupoSujetos.estudio=:Estudio order by varDato.numeroInclucion")
					.setParameter(
							"Entidad",
							this.seguridadEstudio.getEstudioEntidadActivo()
									.getEntidad())
					.setParameter(
							"Estudio",
							this.seguridadEstudio.getEstudioEntidadActivo()
									.getEstudio()).getResultList();

			Long numeroInclusion = null;
			if (numeros.size() == 0) {
				numeroInclusion = Long.parseLong("1");
			} else {
				numeroInclusion = numeros.get(numeros.size() - 1) + 1;
			}*/
			sujeto.setNumeroInclucion(this.numeroInclucion);
			this.sujeto.setCodigoPaciente(inicialesCentroPromotor
					+ "_"
					+ this.seguridadEstudio.getEstudioEntidadActivo()
							.getEstudio().getIdentificador() + "_"
					+ inicialesCentro + "_" + inicialesP + "_"
					+ sujeto.getNumeroInclucion());
			...
			if ((DevolverRol().getCodigo().equals("ecCord") || DevolverRol().getCodigo().equals("ecInv"))
				...
				Date fechaAux = null;
				if (this.sujeto.getFechaInclucion() != null && !this.sujeto.getFechaInclucion().isEmpty()){
					try {
						fechaAux = new SimpleDateFormat("dd/MM/yyyy").parse(this.sujeto.getFechaInclucion());
					} catch (Exception e){
						fechaAux = null;
					}
				}
	...
	public boolean errores() {
		error = false;
		// Date fechaNac = this.sujeto.getFechaNacimiento();
		String codigoSujeto = "";
		...
		try {
			Long cant = (Long) entityManager
					.createQuery(
							"select count(s) from Sujeto_ensayo s where s.codigoPaciente= :codigo and s.eliminado = false")
					.setParameter("codigo", codigoSujeto).getSingleResult();
			if (cant > 0) {
				error = true;
				return true;
			}
