package gehos.configuracion.clinicaldata.departamentos.porentidadnoclinicos;
...
import gehos.configuracion.clinicaldata.departamentos.porentidadnoclinicos.treebuilders.DepartamentosPorEntidadTreeBuilder;
...
@Name("departamentosNoClinicosPorEntidadManagerBulkStyle")
public class DepartamentosPorEntidadManagerBulkStyle {
	...
	@In(required = false, create = true, value = "departamentosNoClinicosPorEntidadTreeBuilder")
	...
	public void subir() {
		if (this.selectedItem instanceof EntidadWrapper) {
			...
			this.selectedItem = new EntidadWrapper(entidad, false, entidad
					.getId());
			...
			for (ServicioWrapper wrapper : visibleServices) {
				if (!wrapper.getExistia() && wrapper.getExiste()) {
					...
					if (!wrapper.getTeniaConsultaExterna()
						...
						cambiarTieneConsultaAServicio(wrapper.getId(), wrapper
								.getEntidadID(), true);
					}
					if (wrapper.getTeniaConsultaExterna()
							&& !wrapper.getTieneConsultaExterna()) {
	...
	public void cambiarTieneConsultaAServicio(Long servId,
			Long entidadId, boolean tiene) {
		ServicioInEntidad_configuracion funct = (ServicioInEntidad_configuracion) entityManager
				.createQuery(
						"from ServicioInEntidad_configuracion dep "
	...
	public void changeEspecialtyVisibility(Long entidadId,
		...
		EspecialidadInEntidad_configuracion funct = (EspecialidadInEntidad_configuracion) entityManager
				...
				.setParameter("entId", entidadId).setParameter("depId",
						especialtyId).getSingleResult();
	...
	public List<DepartamentoWrapper> departments() {
		...
		for (Departamento_configuracion dep : list) {
			...
			wrapper
					.setVisible(depEnt == null ? false
							: (depEnt.getEliminado() == null || depEnt
									.getEliminado() == false));
	...
	public List<ServicioWrapper> services(Long departmentId) {
		...
		for (Servicio_configuracion serv : list) {
			...
			wrapper
					.setVisible(servEnt == null ? false
							: (servEnt.getEliminado() == null || servEnt
									.getEliminado() == false));
	...
	public void setVisibleEspecialties(List<EspecialidadWrapper> visibleEspecialties) {
	...
	public List<EspecialidadWrapper> especialties(Long serviceId) {
		...
		for (Especialidad_configuracion esp : list) {
			...
			wrapper
					.setVisible(espEnt == null ? false
							: (espEnt.getEliminado() == null || espEnt
									.getEliminado() == false));
	...
	@SuppressWarnings("unchecked")
	public ServicioInEntidad_configuracion entidadTieneServicio(
			Long entidadId, Long servicioId) {
	...
	@SuppressWarnings("rawtypes")
	public void setSelectedItem(ITreeData selectedItem, TreeNode node, ITreeData selectedItem2, TreeNode node2) {
		this.selectedItem = null;
