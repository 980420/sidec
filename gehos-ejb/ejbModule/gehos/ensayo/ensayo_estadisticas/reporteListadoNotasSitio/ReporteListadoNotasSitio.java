...
@Name("reporteListadoNotasSitio")
@Scope(ScopeType.CONVERSATION)
public class ReporteListadoNotasSitio{
	...
	private Map reporteListadoNotasSitio;
	...
	//RF3 - Generar reporte de listado de notas de Sitio 
	public void listadoNotasSitio()	{
		
		notasSitio = new ArrayList<NotasSitio>();
		style = -1;
		...
		if(fechaFin != null && fechaInicio != null && !this.estudio.equals("")){
						...
						for(int k=0;k<sujetosDelGrupo.size();k++) {
							...
							if(estadoN.equals("") || estadoN.equals("<Seleccione>")){
								notasSujeto = entityManager
										.createQuery(
												"select nota from Nota_ensayo nota where nota.eliminado = false and nota.notaPadre = null and nota.notaSitio = true and nota.crdEspecifico.momentoSeguimientoEspecifico.sujeto.id=:idSujeto and nota.fechaCreacion >=:fechaInicio and nota.fechaCreacion <=:fechaFin")									
								...
								notasSujeto = entityManager
										.createQuery(
												"select nota from Nota_ensayo nota where nota.eliminado = false and nota.notaPadre = null and nota.notaSitio = true and nota.crdEspecifico.momentoSeguimientoEspecifico.sujeto.id=:idSujeto and nota.fechaCreacion >=:fechaInicio and nota.fechaCreacion <=:fechaFin and nota.estadoNota.nombre=:estadoNota")									
										.setParameter("idSujeto",sujeto.getId())
										.setParameter("fechaInicio",fechaInicio)
							...
							for(int m=0; m < notasSujeto.size(); m++) {
								...
								NotasSitio notaSitioSuj = new NotasSitio(sujeto.getCodigoPaciente(), fechaCreacionNota, nombreCRD, nombreVariable, valorVariable, descripcion,  this.style.toString());
								notasSitio.add(notaSitioSuj);
							}								
						}							 
					}					
				}
			...
			if (haySujetos) {
				...
				reporteListadoNotasSitio=new HashMap();
				reporteListadoNotasSitio.put("nombreEstudio", this.estudio);
				reporteListadoNotasSitio.put("totalSujetos", totalSujetos);
				reporteListadoNotasSitio.put("nombreSujeto", SeamResourceBundle.getBundle().getString("nombreSujeto"));
				reporteListadoNotasSitio.put("fechaCreacionNota", SeamResourceBundle.getBundle().getString("fechaCreacionNota"));
				reporteListadoNotasSitio.put("nombreCRD", SeamResourceBundle.getBundle().getString("nombreCRD"));
				reporteListadoNotasSitio.put("nombreVariable", SeamResourceBundle.getBundle().getString("nombreVariable"));
				reporteListadoNotasSitio.put("valorVariable", SeamResourceBundle.getBundle().getString("valorVariable"));
				reporteListadoNotasSitio.put("descripcionNota", SeamResourceBundle.getBundle().getString("descripcionNota"));
				reporteListadoNotasSitio.put("detallesNota", SeamResourceBundle.getBundle().getString("detallesNota"));
				reporteListadoNotasSitio.put("estadoNota", SeamResourceBundle.getBundle().getString("estadoNota"));		
				nombreReport=reportManager.ExportReport("reportListadoNotasSitio", reporteListadoNotasSitio, notasSitio, FileType.HTML_FILE);
				flag=true;
				flag2=false;
			} 
			else {
	...
	public void exportReportToFileFormat() {
		...
		if (fileformatToExport.equals(filesFormatCombo.get(0))) {
			pathExportedReport = reportManager.ExportReport("reportListadoNotasSitio", reporteListadoNotasSitio, notasSitio, FileType.PDF_FILE);
		} else if (fileformatToExport.equals(filesFormatCombo.get(1))) {
			pathExportedReport = reportManager.ExportReport("reportListadoNotasSitio", reporteListadoNotasSitio, notasSitio, FileType.RTF_FILE);
		} else if (fileformatToExport.equals(filesFormatCombo.get(2))) {
			pathExportedReport = reportManager.ExportReport("reportListadoNotasSitio", reporteListadoNotasSitio, notasSitio, FileType.EXCEL_FILE);
		}
	}
