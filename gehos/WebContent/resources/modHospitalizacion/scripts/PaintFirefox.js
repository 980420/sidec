...
var img2 = new Image();
//var img3 = new Image();
img2.src="/gehos/resources/modHospitalizacion/images/partesCuerpoImages/Untitled-7.jpg";
...
/* Reinicia el editor a su estado inicial*/
	...
	function background_change(valor)
		{
			/*img1.src = "Imagenes/Fondos/fondo"+valor+".jpg";
			if(comprobar ==0)
				{
					img1.src = "Imagenes/Fondos/fondo"+valor+".jpg";
					intervalID = setInterval("Espera()",1);
					comprobar = valor;
				}
			else
				 {
	...
	function Open_Imagen(dir)
		{
			img2.src ="file:///"+dir;
			intervalID2 = setInterval("Img_()",1);
	 	}
	...
	function Img_()
		{
			if(img2.complete)
				{
					var oCanvas2 = document.getElementById("canvas").getContext("2d");
					oCanvas2.drawImage(img2,0,0);
	...
	function Espera()
		...
		if(img1.complete)
		{
                ...
                oCanvas.drawImage(img1,50,0);
				oCanvas.save();
				clearInterval(intervalID);
			}
		}

	/* Agrupa los datos y hace una llama asincrona para guardar la imagen*/
	function Save_all()
		{
			var datai=viewsvg();
		   document.imagen.imagenvalor.value=datai;
		   var image_name=document.imagen.nombre.value;
		   var query_string="imagenvalor="+datai+" & nombre="+image_name+" & direccion="+document.imagen.direccion.value;
		   llamarasincrono(direccion_destino, 'workspace',query_string);
  ...
  cundo = canvasundo.getContext("2d");
	...
	/* Permite cambiarle el tamaño a las autoformas y letras*/
	...
	function shortcuts(e)
	{
		if(e.keyCode == 46)
			{
				if(c.tool.name == 'select' && c.tool.status > 0)
					{
						c.tool.del();
					}
			}
	...
	function sel_cancel()
	{
		if(c.tool.status == 2) {//cambios aqui
	...
	c.lineWidth = 3;
       	...
       	_brushes: function() {
			...
			this.up = function() {
				...
				this.sstart = null;
				this.status = 0;
                	iface.status.innerHTML = 'up';
			}
			this.draw = function() {
                    	iface.status.innerHTML = 'pintando';
				if(prefs.pretty) {
	...
	Lapiz: function() {
           document.getElementById("workspace").style.cursor="url('images/Cursores/lapiz.cur'),move";
		this.name = 'Lápiz';
		this.status = 0;
		this.inherit = tool._brushes; this.inherit();
		//c.lineWidth = 12;
	...
	Borrar: function() {
          document.getElementById("workspace").style.cursor="url('images/Cursores/goma_de_borrar.cur'),move";
		this.name = 'eraser';
		this.status = 0;
		this.inherit = tool._brushes; this.inherit();
...
c.lineWidth = 32;
	...
	Linea: function() {
              document.getElementById("workspace").style.cursor="url('images/Cursores/linea_recta.cur'),move";
		...
		this.move = function(e) {
			this._move();
			drawLine(this.start.x+document.body.scrollLeft, this.start.y+document.body.scrollTop, m.x+document.body.scrollLeft, m.y+document.body.scrollTop, e.shiftKey, ctemp);
		}
		this.up = function(e) {
			this._up();
			drawLine(this.start.x+document.body.scrollLeft, this.start.y+document.body.scrollTop, m.x+document.body.scrollLeft, m.y+document.body.scrollTop, e.shiftKey, c);
		}
c.lineWidth = 3;
	},
	...
	Cuadrado: function() {
                 document.getElementById("workspace").style.cursor="url('images/Cursores/rectangulo.cur'),move";
		...
		this.move = function(e) {
			this._move();
			ctemp.clearRect(0, 0, canvastemp.width, canvastemp.height);
			drawRectangle(this.start.x+document.body.scrollLeft, this.start.y+document.body.scrollTop, m.x+document.body.scrollLeft, m.y+document.body.scrollTop, e.shiftKey, ctemp);
		...
		this.up = function(e) {
			this._up();
			drawRectangle(this.start.x+document.body.scrollLeft,this.start.y+document.body.scrollTop, m.x+document.body.scrollLeft, m.y+document.body.scrollTop, e.shiftKey, c);
		}
c.lineWidth = 3;
	},
	...
	Circulo: function() {
          ...
          document.getElementById("workspace").style.cursor="url('images/Cursores/elipse.cur'),move";
		this.name = 'ellipse';
		this.status = 0;
		this.inherit = tool._shapes; this.inherit();
		...
		this.move = function(e) {
			this._move();
			drawEllipse(this.start.x+document.body.scrollLeft, this.start.y+document.body.scrollTop, m.x+document.body.scrollLeft, m.y+document.body.scrollTop, e.shiftKey, ctemp);
		}
		this.up = function(e) {
			this._up();
			drawEllipse(this.start.x+document.body.scrollLeft, this.start.y+document.body.scrollTop, m.x+document.body.scrollLeft, m.y+document.body.scrollTop, e.shiftKey, c);
...
c.lineWidth = 3;
	...
	rounded: function() {
          document.getElementById("workspace").style.cursor="url('images/Cursores/rectangulo_redondeado.cur'),move";
		...
		this.move = function(e) {
			this._move();
			drawRounded(this.start.x+document.body.scrollLeft, this.start.y+document.body.scrollTop, m.x+document.body.scrollLeft, m.y+document.body.scrollTop, e.shiftKey, ctemp);
		}
		this.up = function(e) {
			this._up();
			drawRounded(this.start.x+document.body.scrollLeft, this.start.y+document.body.scrollTop, m.x+document.body.scrollLeft, m.y+document.body.scrollTop, e.shiftKey, c);
		}
c.lineWidth = 3;
	},
	...
	Curva: function() {
          document.getElementById("workspace").style.cursor="url('images/Cursores/linea_curva.cur'),move";
		...
		this.down = function() {
			if(this.status==0) { //starting
				undoSave();
				activateTempCanvas();
				this.start = { x:m.x+document.body.scrollLeft, y:m.y+document.body.scrollTop }
				this.end = null;
				this.bezier1 = null;
				this.status = 5;
				c.beginPath();
...
ctemp.beginPath();
		...
		this.up = function() {
			if(this.status==5) { //setting endpoint     // && source.id != 'body'
				this.end = { x:m.x+document.body.scrollLeft, y:m.y+document.body.scrollTop }
				this.status = 4;
				//linea
			} else if(this.status==3) { //setting bezier1  && source.id != 'body'
				//curva cuando la suelto

				this.bezier1 = { x:m.x, y:m.y }
				ctemp.clearRect(0, 0, canvastemp.width, canvastemp.height);
				ctemp.moveTo(this.start.x, this.start.y);
				ctemp.bezierCurveTo(m.x+document.body.scrollLeft, m.y+document.body.scrollTop, this.end.x, this.end.y, this.end.x, this.end.y);
				ctemp.stroke();
				...
				c.bezierCurveTo(m.x+document.body.scrollLeft,m.y+document.body.scrollTop, this.end.x, this.end.y, this.end.x, this.end.y);
				c.stroke();
				this.status = 0;
				iface.txy.innerHTML = '&nbsp;';
...
c.lineWidth = 3;
   	...
   	Seleccion: function() {
            document.getElementById("workspace").style.cursor="url('images/Cursores/seleccion.cur'),move";
        ...
        if(this.status==0) { //starting select
				var dashed =dash; //doing this here instead of in selTool to ignore user changing color
				//dashed.src = 'images/dashed2.gif';
				c.strokeStyle = c.createPattern(dashed, 'repeat');
				activateTempCanvas();
				this.start = { x:m.x, y:m.y }
				this.status = 4;
                document.getElementById("status").innerHTML="m1";
             ...
             { c.strokeStyle = '#000';
               c.lineWidth = 3;
				...
				if(intersects(m, this.start, this.dimension)) {
				  document.getElementById("status").innerHTML="m2";
					this.offset = { x:m.x-this.start.x, y:m.y-this.start.y }
					if(this.status == 3 && !e.ctrlKey && !e.shiftKey) {
					  document.getElementById("status").innerHTML="m3";   //when first moving (and not in stamp mode), clear original pos and paint on tempcanvas
						undoSave();
					...
					if(this.status < 3) {
					  //actually draw last moved selection to canvas TODO also do this when switching tools
                      document.getElementById("status").innerHTML="m4";
						c.drawImage(canvassel, Math.floor(this.start.x), Math.floor(this.start.y));
					}
                    document.getElementById("status").innerHTML="m5";
					activateTempCanvas();
					this.start = { x:m.x, y:m.y }
					this.status = 4;
				}
