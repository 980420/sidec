package gehos.configuracion.clinicaldata.camas.porubicaciones.treebuilders;

import gehos.configuracion.clinicaldata.ubicaciones.treebuilders.model.CamaWrapper;
...
//							        + "where ent.perteneceARhio = true "
	...
	private boolean expandUbicacion(TreeNode selected) {
		...
		List<Cama_configuracion> camas = entityManager.createQuery(
				"from Cama_configuracion c where c.ubicacion.id = :pid")
				.setParameter("pid", value.getValue().getId()).getResultList();

		for (int i = 0; i < camas.size(); i++) {
			TreeNode camaNode = new TreeNodeImpl();
			CamaWrapper w = new CamaWrapper(camas.get(i), false);
			camaNode.setData(w);

			selected.addChild(w.hashCode(), camaNode);
		}

		((ITreeData) selected.getData()).setExpanded(true);
		return (ubicaciones.size() + camas.size()) > 0;
	}
