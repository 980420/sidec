...
@Name("departamentoClinicoBuscarControler")
public class DepartamentoClinicoBuscarControler extends
		EntityQuery<Departamento_configuracion> {

	private static final String EJBQL = "select departamento from Departamento_configuracion departamento where departamento.esClinico = true";

	private static final String[] RESTRICTIONS = { "lower(departamento.nombre) like concat(lower(#{departamentoClinicoBuscarControler.nombre.trim()}),'%')",
	// "#{departamentoClinicoBuscarControler.departamentoId} <> departamento.id"
	};
	...
	// Methods ----------------------------------------------------
	public DepartamentoClinicoBuscarControler() {
		setEjbql(EJBQL);
	...
	public void eliminar() {
		try {
			...
			facesMessages
					.addToControl("error", Severity.ERROR,
							"El departamento clínico está en uso por lo tanto no puede ser eliminado.");
