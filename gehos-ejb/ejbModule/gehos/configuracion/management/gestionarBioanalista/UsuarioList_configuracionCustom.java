...
@Name("usuarioBioanalistaList_configuracionCustom")
@Scope(ScopeType.CONVERSATION)
public class UsuarioList_configuracionCustom extends EntityQuery<Usuario_configuracion> {	
	private static final long serialVersionUID = -5113501274620536673L;

	@Override
	public String getEjbql() {
		return "select user from Usuario_configuracion user where user.id not in (select medico.id from Bioanalista_configuracion medico) and user.id not in (select enfermero.id from Enfermera_configuracion enfermero)";
	}
	
	private static final String[] RESTRICTIONSA = {
		
		"lower(user.nombre) like concat(lower(#{usuarioBioanalistaList_configuracionCustom.nombre.trim()}),'%')",
		"lower(user.username) like concat(lower(#{usuarioBioanalistaList_configuracionCustom.username.trim()}),'%')",		
		"lower(user.primerApellido) like concat(lower(#{usuarioBioanalistaList_configuracionCustom.primerapellido.trim()}),'%')",
		"lower(user.segundoApellido) like concat(lower(#{usuarioBioanalistaList_configuracionCustom.segundoapellido.trim()}),'%')",
	...
	"lower(user.username) like concat(lower(#{usuarioBioanalistaList_configuracionCustom.nicik.trim()}),'%')",	
	};
