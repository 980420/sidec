...
@Name("trazasControladora1")
public class CopyOfTrazasControladora {
...
//	private List<String> ips = new ArrayList<String>();
	...
	public List<String> llenarUsers() {
	   ...
	   //Listado de los estudios
		...
		private String estudioN;
		
		
		// llenar los estudios que se puede buscar
...
//	}
	...
	// Se implementa el requisito funcional 5: Permite exportar las trazas
	...
	// Aqui se implementa el requisito funcional 4:Visualizar trazas de usuarios
	@SuppressWarnings("unchecked")
	...
	// Metodo para creacion del arbol again
	@In
	...
	public void inicializarConjuntoDatos() {
		for (int j = 0; j < listaEstudioEntidad.size(); j++) {
            if(listaEstudioEntidad.get(j).getNombre().equals(this.estudioN)){
                    estudio = listaEstudioEntidad.get(j);
                    break;
            }}
	...
	public List<MomentoSeguimientoGeneralHojaCrd_ensayo> hojasMomentos(MomentoSeguimientoGeneral_ensayo momento) {
		...
		hojas = entityManager.createQuery("select msHojaCrd from MomentoSeguimientoGeneralHojaCrd_ensayo msHojaCrd "
				...
				+ "order by msHojaCrd.id").setParameter("idMomento", momento.getId()).getResultList();
		return hojas;
	}
	...
	public Estudio_ensayo estudiosel(String estudio){
		Estudio_ensayo estudiosel= (Estudio_ensayo) entityManager.createQuery("select estudio from Estudio_ensayo estudio "
				+ "where estudio.nombre =:nombre " +
				  "and (estudio.eliminado=false or estudio.eliminado=null)").setParameter("nombre", estudio)
				  .getSingleResult();
