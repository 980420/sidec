...
@Name("departamentoNoClinicoCrearControlador")
@Scope(ScopeType.CONVERSATION)
public class DepartamentoNoClinicoCrearControlador {
	...
	public void begin() {
		try {
			...
			this.departamento.setEsClinico(false);
	...
	public boolean validacion() {
		error = 0;
		List<Departamento_configuracion> d = entityManager
				.createQuery(
						"select d from Departamento_configuracion d "
								+ "where d.nombre =:nombre")
				.setParameter("nombre", this.nombre.trim()).getResultList();
		if (d.size() != 0) {
	...
	public void crear() {
		...
		try {
			...
			facesMessages.addToControl("error", Severity.ERROR,
					"El departamento no clínico ha sido creado previamente.");
		}
	}
