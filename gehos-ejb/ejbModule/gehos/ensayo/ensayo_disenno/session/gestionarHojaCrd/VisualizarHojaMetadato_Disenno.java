...
@Name("visualizarHojaMetadato_Disenno")
@Scope(ScopeType.CONVERSATION)
public class VisualizarHojaMetadato_Disenno {
	...
	@SuppressWarnings("unchecked")
	public List<GrupoVariables_ensayo> listadoGrupoVariablesA(Seccion_ensayo seccion) {
		grupoVariables = new ArrayList<GrupoVariables_ensayo>();
		grupoVariables = (List<GrupoVariables_ensayo>) entityManager
				.createQuery(
						"select grup from GrupoVariables_ensayo grup where grup.seccion=:Seccion and grup.eliminado <> true")
	...
	public List<GrupoVariables_ensayo> listadoGrupoVariablesTotal() {
		...
		for (int i = 0; i < listadoSeccionesA().size(); i++) {
			...
			grupoVariablesaux=listadoGrupoVariablesA(listadoSeccionesA().get(i));
	...
	public List<Variable_ensayo> listadoVariablesA(Seccion_ensayo seccion) {
		...
		variables = (List<Variable_ensayo>) entityManager
				.createQuery(
						"select var from Variable_ensayo var where var.seccion=:Seccion and var.eliminado <> true")
				.setParameter("Seccion", seccion).getResultList();
		return variables;
	}
	...
	public String SioNo(boolean bool) {
		...
		String result=(SeamResourceBundle.getBundle().getString(
				"prm_no_ens"));
		if (bool) {
			result=(SeamResourceBundle.getBundle().getString(
							"prm_si_ens"));
		}
		return result;
	}
	...
	public List<Variable_ensayo> listadoVariables(Seccion_ensayo seccion) {
		...
		variables = (List<Variable_ensayo>) entityManager
				.createQuery(
						"select var from Variable_ensayo var where var.seccion=:Seccion")
				.setParameter("Seccion", seccion).getResultList();
		return variables;
	}
	...
	public List<GrupoVariables_ensayo> listadoGrupoVariables(Seccion_ensayo seccion) {
		...
		grupoVariables = (List<GrupoVariables_ensayo>) entityManager
				.createQuery(
						"select grup from GrupoVariables_ensayo grup where grup.seccion=:Seccion")
				.setParameter("Seccion", seccion).getResultList();
	...
	/*public String concatenarId(long idGrupo, long idSeccion){
		return String.valueOf(idGrupo) + String.valueOf(idSeccion); 
	}*/
	public String concatenarId(long idGrupo, long idSeccion) {
	...
	public List ValoresRadio(Nomenclador_ensayo nomenc) {
		...
		lista = (List<NomencladorValor_ensayo>) entityManager
				.createQuery(
						"select nom from NomencladorValor_ensayo nom where nom.nomenclador=:nomenc and nom.valor != null")
	...
	public List<String> Valores(Nomenclador_ensayo nomenc) {
		...
		lista = (List<String>) entityManager
				.createQuery(
						"select nom.valor from NomencladorValor_ensayo nom where nom.nomenclador=:nomenc")
				.setParameter("nomenc", nomenc).getResultList();
		return lista;
	}
	...
	public List<Variable_ensayo> tieneVariableGrupoSeccion(Seccion_ensayo seccion, GrupoVariables_ensayo grupo) {
		...
		try {
			lista = (List<Variable_ensayo>) entityManager.createQuery(
							"select var from Variable_ensayo var where var.grupoVariables=:Grupo and var.seccion=:Seccion")
