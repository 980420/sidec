package gehos.ensayo.ensayo_estadisticas.reporteNotificaciones;
		...
		if (fechaFin != null && fechaInicio != null && !r[0]) {
					...
					if (listadoEstadosSelecionadas
							...
							if (listadoEntidadesSeleccionadas.get(t)
									...
									//if (listadoEstudiosSeleccionados.get(s).getEntidad().getId() == listadoEntidadesSeleccionadas.get(t).getId()) 
									{
											...
											if (listadoGruposSeleccionados.get(g).getEstudio().getId() == listadoEstudiosSeleccionados.get(s).getId()) 
											{
												...
												if(listadoEntidadesSeleccionadas.get(t).getTipoEntidad().getId()==7)
												{
													sujetosDelGrupo = entityManager.createQuery(
															"select distinct sujeto from Sujeto_ensayo sujeto where sujeto.grupoSujetos.id =:idGrupo and sujeto.grupoSujetos.estudio.id =:idEstudio and sujeto.eliminado = false  and sujeto.fechaCreacion>=:fechaInicio and sujeto.fechaCreacion<=:fechaFin and sujeto.id in(select distinct re.idSujeto from Notificacion_ensayo re where re.variableDato.eliminado = FALSE) ")
												...
												if (!sujetosDelGrupo.isEmpty()) 
												{
													...
													for (int k = 0; k < sujetosDelGrupo	.size(); k++) {
														if (sujetosDelGrupo.get(k).getGrupoSujetos().getId() == listadoGruposSeleccionados.get(g).getId()) 
														{
															...
															List<Long> cantNoti = entityManager.createQuery("select re.idSujeto from Notificacion_ensayo re where re.idSujeto=:IdSuj and re.variableDato.eliminado = FALSE").setParameter("IdSuj", sujeto.getId()).getResultList();
															
															notificaciones = cantNoti.size();
																														
															List<MomentoSeguimientoEspecifico_ensayo> momentosSujeto = entityManager.createQuery(
																	...
																	.setParameter("idSujeto",sujeto.getId()).setParameter("fechaFin",fechaFin)
																	.getResultList();
															for (int l = 0; l < momentosSujeto.size(); l++) 
															{
																...
																List<CrdEspecifico_ensayo> crdMomento = entityManager.createQuery(
																...
																for (int m = 0; m < crdMomento.size(); m++) 
																{
																	...
																	for (int i = 0; i < notas.size(); i++) {
																		if(notas.get(i).getEstadoNota().getCodigo() == 1 || notas.get(i).getEstadoNota().getCodigo() == 2){
																			notasSinResolver ++;
																		}
																	}
																}
															...
															Sujeto sujetoEstadisticas = new Sujeto(
																	...
																	momentosIniciados,
																	momentosCompletados,
																	notasSinResolver, momentosNoIni, notificaciones);
												...
												if (!sujetos.isEmpty()) {
													GrupoSujeto grupoNuevo = new GrupoSujeto(
															...
															totalNotasSinResolver, totalMomentosNoIni, totalNoti);
													grupoSujetos
															.add(grupoNuevo);
										...
										if (!grupoSujetos.isEmpty()) {
											Ensayo ensayo = new Ensayo(
													...
													totalMIEstudio,
													totalMCEstudio,
													totalNSREstudio, totalMNoIEstudio, totalNotiEst);
											estudios.add(ensayo);
								...
								if (!estudios.isEmpty()) {
									Entidad entidad = new Entidad(
											...
											totalMCEntidad, totalNSREntidad, totalMNoIEntidad, totalNotiEntidad);
									...
									totalNotiEstado += totalNotiEntidad;
								}
						...
						if (!entidades.isEmpty()) {
							Provincia provincia = new Provincia(
									...
									totalNSREstado, totalMNoIEstado, totalNotiEstado);
							...
							totalNotiNacion += totalNotiEstado;
						}

					}

				}
				if(totalMINacion==0 && totalMINacion==0 && totalNSRNacion==00 && totalMNoINacion==0)
					break;
				if (estados != null && haySujetos) {
					Pais pais = new Pais(listadoNacionSelecionadas.get(n).getValor(), estados, totalMINacion, totalMCNacion,totalNSRNacion, totalMNoINacion, totalNotiNacion);
					naciones.add(pais);
				}
			...
			if (haySujetos) {
				...
				reporteMonitoreo.put("grupo", SeamResourceBundle.getBundle()
						.getString("grupo"));
				reporteMonitoreo.put("cantNotificaciones", SeamResourceBundle.getBundle()
						.getString("cantNotificaciones"));
				reporteMonitoreo.put("nombreSujeto", SeamResourceBundle
						.getBundle().getString("nombreSujeto"));
				...
				nombreReport = reportManager.ExportReport(
						"reportEstadisticaNotificaciones", reporteMonitoreo,
