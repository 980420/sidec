...
@Name("sujetoCustomListPesquisaje")
@Scope(ScopeType.CONVERSATION)
public class SujetoCustomListPesquisaje extends SujetoList_ensayo {

	private static final String EJBQL = "select sujeto from Sujeto_ensayo sujeto "
			+ "where sujeto.eliminado = false and sujeto.grupoSujetos.estudio = #{sujetoCustomListPesquisaje.seguridadEstudio.getEstudioEntidadActivo().getEstudio()} ";

	private static final String[] RESTRICTIONS = {
			"lower(sujeto.codigoPaciente) like concat('%',concat(lower(#{sujetoCustomListPesquisaje.sujeto.codigoPaciente.trim()}),'%'))",
			"#{sujetoCustomListPesquisaje.idSujeto} <> sujeto.id" };
	...
	public void init() {
		if(!this.pesquisajeHabilitado()){
			this.facesMessages.addToControlFromResourceBundle("idForm",Severity.INFO,"iniciales");
		}
	...
	public List<Sujeto_ensayo> getResultList() {
		...
		if (!this
			...
			String tempEJB = "select sujeto from Sujeto_ensayo sujeto "
					+ "where sujeto.eliminado = false and sujeto.grupoSujetos.estudio = #{sujetoCustomListPesquisaje.seguridadEstudio.getEstudioEntidadActivo().getEstudio()} "
					+ "and sujeto.entidad = #{sujetoCustomListPesquisaje.getHospitalActivo()} ";
	...
	public SujetoCustomListPesquisaje() {
		setEjbql(EJBQL);
	...
	public MomentoSeguimientoEspecifico_ensayo obtenerMomentoPesquisaje(long subjectID){
		MomentoSeguimientoEspecifico_ensayo momentoPesquisaje = null;
		try {
			@SuppressWarnings("unchecked")
			List<MomentoSeguimientoEspecifico_ensayo> listamomentoPesquisaje = (List<MomentoSeguimientoEspecifico_ensayo>)entityManager
					.createQuery(
							"select mom from MomentoSeguimientoEspecifico_ensayo mom where mom.sujeto.id=:idSuj and mom.momentoSeguimientoGeneral.programado = true and mom.eliminado = false and mom.momentoSeguimientoGeneral.nombre = 'Pesquisaje'")
					.setParameter("idSuj", subjectID).getResultList();
			momentoPesquisaje = listamomentoPesquisaje.get(0);
		} catch (Exception e) {
			return null;
		}
		return momentoPesquisaje;
	}
