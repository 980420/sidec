...
@Name("sujetoCustomList")
@Scope(ScopeType.CONVERSATION)
public class SujetoCustomList extends SujetoList_ensayo {

	private static final String EJBQL = "select sujeto from Sujeto_ensayo sujeto "
			+ "where sujeto.eliminado = false and (sujeto.estadoInclusion.codigo = 4 or sujeto.estadoInclusion.codigo = 2) and sujeto.grupoSujetos.estudio = #{sujetoCustomList.seguridadEstudio.getEstudioEntidadActivo().getEstudio()} ";
	
	private static final String EJBQL_DASH = "select sujeto from Sujeto_ensayo sujeto "
			+ "where sujeto.eliminado = false and sujeto.estadoInclusion.codigo = 4 and sujeto.grupoSujetos.estudio = #{sujetoCustomList.seguridadEstudio.getEstudioEntidadActivo().getEstudio()} ";

	private static final String[] RESTRICTIONS = {
			"lower(sujeto.codigoPaciente) like concat('%',concat(lower(#{sujetoCustomList.sujeto.codigoPaciente.trim()}),'%'))",
			"lower(sujeto.grupoSujetos.nombreGrupo) like concat(lower(#{sujetoCustomList.nombeGrup.trim()}),'%')",
			"lower(sujeto.estadoInclusion.nombre) like concat(lower(#{sujetoCustomList.estadoI.trim()}),'%')",
			"lower(sujeto.estadoTratamiento.nombre) like concat(lower(#{sujetoCustomList.estadoT.trim()}),'%')",
			"lower(sujeto.fechaCreacion) like concat(lower(#{sujetoCustomList.fechaInicioA.trim()}),'%')",
			"lower(sujeto.fechaCreacion) like concat(lower(#{sujetoCustomList.fechaInicioP.trim()}),'%')",
			"#{sujetoCustomList.idSujeto} <> sujeto.id" };
	...
	public List<Sujeto_ensayo> getResultList() {
		...
		if (!this
			...
			String tempEJB = "select sujeto from Sujeto_ensayo sujeto "
					+ "where sujeto.eliminado = false and (sujeto.estadoInclusion.codigo = 4 or sujeto.estadoInclusion.codigo = 2) and sujeto.grupoSujetos.estudio = #{sujetoCustomList.seguridadEstudio.getEstudioEntidadActivo().getEstudio()} "
					+ "and sujeto.entidad = #{sujetoCustomList.getHospitalActivo()} ";
	...
	public String estadoMonitoreo(Long idSujeto) {
		List<MomentoSeguimientoEspecifico_ensayo> listarMomentos;
		
		Sujeto_ensayo sujeto = (Sujeto_ensayo) entityManager
				.createQuery(
						"select sujeto from Sujeto_ensayo sujeto where sujeto.eliminado = false and sujeto.id=:idSuj")
				.setParameter("idSuj", idSujeto).getSingleResult();
		
		// Si el sujeto esta interrumpido 
		// los MS con fecha posterior a la interrupcion no los tengo en cuenta
		boolean esSujetoInterrumpido=sujeto.getFechaInterrupcion()!=null;
		
		if(esSujetoInterrumpido){
			listarMomentos = (List<MomentoSeguimientoEspecifico_ensayo>) entityManager
					.createQuery(
							"select mom from MomentoSeguimientoEspecifico_ensayo mom where mom.fechaInicio <= mom.sujeto.fechaInterrupcion and mom.eliminado = false and mom.sujeto.id=:idSuj")
					.setParameter("idSuj", idSujeto).getResultList();
		}else{
			listarMomentos = (List<MomentoSeguimientoEspecifico_ensayo>) entityManager
					.createQuery(
							"select mom from MomentoSeguimientoEspecifico_ensayo mom where mom.eliminado = false and mom.sujeto.id=:idSuj")
					.setParameter("idSuj", idSujeto).getResultList();
		}
	...
	public List<String> listEstadoI() {
		...
		for (int i = 0; i < listarEstados.size(); i++) {
			if(listarEstados.get(i).getCodigo() == 4 || listarEstados.get(i).getCodigo() == 2){
	...
	public Long obtenerSujetosIncluidos() {
	 	    
	    return  (long) getResultList().size();
	}
	
	public Integer totalSujetosEstudio(){
		
		Integer cantSujetos=0;		
		List<Sujeto_ensayo> sujetosDelEstudio = entityManager
				.createQuery("select sujeto from Sujeto_ensayo sujeto "
					       + "where sujeto.eliminado = false and sujeto.estadoInclusion.codigo = 4 and sujeto.grupoSujetos.estudio = #{sujetoCustomList.seguridadEstudio.getEstudioEntidadActivo().getEstudio()} and sujeto.grupoSujetos.nombreGrupo <> 'Grupo Validación'")				
				.getResultList();
		
		cantSujetos=sujetosDelEstudio.size();
